...
const Invest = ({ userDetail }: InvestProps) => {
  ...
  const investUSDC = async () => {
    ...
    await toast.promise(_investUSDC(userDetail.eth_address!, amount), {
      pending: `Investing ${amount}...`,
      success: 'Invested successfully 👌',
      error: 'Investment rejected 🤯'
    });
    // let toastId = null;
    // try {
    //   toastId = toast.loading(`Investing ${amount}...`);
    //   await _investUSDC(userDetail.eth_address!, amount);
    ...
    //     toast.update(toastId!, {
    //       render: 'Gained some ETH. Please try again after 30s',
    //       type: 'info',
    //       isLoading: false,
    //       autoClose: 5000
    //     });
